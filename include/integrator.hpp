...
class Integrator {
    public: 
        Integrator(const Dynamics_Abstract<T, nx, nu, type>& dynamics_, type dt_);
    ...
    protected:
        type dt;
        const Dynamics_Abstract<T, nx, nu, type>& dynamics; 
};

template <int T, int nx, int nu, typename type=float>
class EulerIntegrator : public Integrator<T, nx, nu, type> {
    public:
        EulerIntegrator(const Dynamics_Abstract<T, nx, nu, type>& dynamics_, type dt_);
